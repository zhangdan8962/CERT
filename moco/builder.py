...
class SB_MoCo(nn.Module):
    ...
    def __init__(self, dim=300, K=65536, m=0.999, T=0.07, mlp=False):
        ...
        encoder_q = SentenceTransformer('bert-base-nli-mean-tokens')

        encoder_k = SentenceTransformer('bert-base-nli-mean-tokens')
    ...
    def forward(slef,feature_q,feature_k):
        """
        Input:
            im_q: a batch of query images
            im_k: a batch of key images
        ...
        # compute query features
        q = self.encoder_q(feature_q)  # queries: NxC
        q = nn.functional.normalize(q['sentence_embedding'], dim=1)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            k = self.encoder_k(feature_k)  # keys: NxC
            k = nn.functional.normalize(k['sentence_embedding'], dim=1)
