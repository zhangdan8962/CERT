    ...
    for i,  batch in enumerate(train_loader):
        ...
        for paired_sentence_idx in range(len(features)):
            for feature_name in features[paired_sentence_idx]:
                features[paired_sentence_idx][feature_name] = features[paired_sentence_idx][feature_name].cuda(args.gpu,non_blocking=True)

    
        # compute output
        output, target = model(features[0],features[1])
